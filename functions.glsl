...
vec3 lightningCalcutations(vec3 albedo){
    ...
    vec3 lightColor = pow(texture(lightmap, lightMapCoords).rgb, vec3(2.2)); // linear -> gamma; light power
    ...
    vec3 outputColor = albedo * ambientLight + BRDF(shadowLightDirection, viewPosition, roughness, normalWorldSpace, albedo, metallic, reflectance, false, false, lightColor);
    //light
